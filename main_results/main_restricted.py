...
def main(
    ...
    with open(output_path, "a") as f:
        for i in trange(0, len(data), batch_size):
            ...
            images = [Image.open(item["image"]) for item in batch if classes.index(item["label"]) in range(10)]
...
def main_clip(
    ...
    random.seed(seed)
    random.shuffle(data)
    classes = json.load(open(class_path))[0:10]
    data = data
    ...
    with open(output_path, "w") as f:
        for i in trange(0, len(data), batch_size):
            ...
            images = [
                ...
                for item in batch if item["label"] in classes
            ]
            idxs = [
                idx
                for idx,item in enumerate(batch) if item["label"] in classes
            ]
            ...
            text_descriptions = [f"A photo of a car {cls}" for cls in classes]
            ...
            for idx, sampled_idx in enumerate(idxs):
                item = batch[sampled_idx]
                if including_label:
                    choice = choices[idx]
                    ...
                    predicted_class = [classes[i] for i in similarities[idx].argsort().flip(dims=[0])[0:5]]
