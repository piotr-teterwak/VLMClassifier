...
def main(model_id, data_path, class_path, split, seed, output_path):
    ...
    with open(output_path, "a") as f:
        for idx in trange(len(data)):
             if all_classes.index(data[idx]["label"]) in [1,2]:
                 ...
                 if "blip" in model_id:
                     ...
                     choices = '\n'.join(['{}. {}'.format(c_idx,c) for c_idx,c in  enumerate(classes)])
                     prompt = f"User:<image>What is the year, make, and model of  the vehicle in this photo? Choose one from: \n {choices}.<end_of_utterance>\nAssistant: The year, make, and model of the vehicle in this photo is "
                 ...
                 data[idx]["label_index"] = all_classes.index(data[idx]["label"]) - 1
