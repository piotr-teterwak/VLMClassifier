...
def main(model_id, data_path, class_path, split, seed, output_path):
    ...
    classes_joined = classes_1 + classes_2
    #data = data[:1000]
        ...
        for idx in trange(len(data)):
             if all_classes.index(data[idx]["label"]) in range(10):
                 ...
                 if "blip" in model_id:
                     ...
                     choices_1 = '\n'.join(['{} {}'.format(c_idx,c) for c_idx,c in  zip(['A.', 'B.', 'C.', 'D.', 'E.'],classes_1)])
                     choices_2 = '\n'.join(['{} {}'.format(c_idx,c) for c_idx,c in  zip(['A.', 'B.', 'C.', 'D.', 'E.'],classes_2)])
...
5. Select the most likely option:\n\
 ...
 \nChoices:\n{choices_2}.\nAnswer with the letter.<end_of_utterance>\nAssistant: Answer: "
                     ...
                     top_5_indices = np.array([p[0] for p in probs]).argsort()[-5:][::-1]
                     top_5_classes = [classes_joined[i] for i in top_5_indices]
                     top_5_choices = '\n'.join(['{}. {}'.format(chr(65 + i), top_5_classes[i]) for i in range(5)])
                     prompt_3 = f"User:<image> Question: Identify the year, make, and model of the car in the given image. Please select the correct option from the multiple choices provided below.\n Steps to identify:\n\
1. Observe the car's features:\n\
   Look at the shape of the body, headlights, grille, and other distinguishing features.\n\
   \n\
2. Compare with common characteristics of makes and models:\n\
...
5. Select the most likely option:\n\
 ...
 \nChoices:\n{top_5_choices}.\nAnswer with the letter.<end_of_utterance>\nAssistant: Answer: "
                     ...
                     for i, p_idx in enumerate(top_5_indices):
                         probs[p_idx] = probs_3[i]
