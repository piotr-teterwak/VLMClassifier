...
def main(model_id, data_path, class_path, split, seed, output_path):
    ...
    classes_1 = [all_classes[i] for i in [0,4,5,6,9]]
    classes_2 = [all_classes[i] for i in [8,2,3,1,7]]
    #data = data[:1000]

    print(f"{len(data)=}")
    print(f"{len(set(classes_1))=}")
        ...
        for idx in trange(len(data)):
             if all_classes.index(data[idx]["label"]) in range(10):
                 ...
                 if "blip" in model_id:
                     ...
                     choices_1 = '\n'.join(['{} {} OR {}'.format(c_idx,c1,c2) for c_idx,c1,c2 in  zip(['A.', 'B.', 'C.', 'D.', 'E.'],classes_1, classes_2)])
                     prompt_1 = f"User:<image> Question: Identify the year, make, and model of the car in the given image. Please select the correct option from the multiple choices provided below.\n Steps to identify:\n\
...
5. Select the most likely option:\n\
 ...
 \nChoices:\n{choices_1}.\nAnswer with the letter.<end_of_utterance>\nAssistant: Answer: "
                     ...
                     first_group_index = sorted(range(len(probs_1)), key=lambda i: probs_1[i], reverse=True)[0]
                     second_group_index = sorted(range(len(probs_1)), key=lambda i: probs_1[i], reverse=True)[1]
                     grouped_classes = list(zip(classes_1,classes_2))
                     selected_classes = grouped_classes[first_group_index] + grouped_classes[second_group_index]



                     # Ungroup the top classes for the second choice
                     choices_2 = '\n'.join(['{} {}'.format(c_idx,c1) for c_idx,c1 in  zip(['A.', 'B.', 'C.', 'D.'],selected_classes)])
                     prompt_2 = f"User:<image> Question: Identify the year, make, and model of the car in the given image. Please select the correct option from the multiple choices provided below.\n Steps to identify:\n\
1. Observe the car's features:\n\
   Look at the shape of the body, headlights, grille, and other distinguishing features.\n\
   \n\
2. Compare with common characteristics of makes and models:\n\
...
5. Select the most likely option:\n\
 ...
 \nChoices:\n{choices_2}.\nAnswer with the letter.<end_of_utterance>\nAssistant: Answer: "
                     ...
                     score_vector = {cls: 0 for cls in classes_1 + classes_2}
                     for pidx, group in enumerate(grouped_classes):
                        for cls in group:
                            score_vector[cls] = probs_1[pidx]

                     for pidx, cls in enumerate(selected_classes):
                         score_vector[cls] = probs_2[pidx]

                     1/0
                     probs = [v for v in score_vector.values()]
