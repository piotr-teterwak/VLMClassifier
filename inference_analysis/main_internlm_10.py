...
def chat_likelihood(
    ...
    **kwargs,
    ...
    if image is None:
        ...
        inputs, im_mask, _ = model.interleav_wrap_chat(query, image, history=[], meta_instruction=meta_instruction, hd_num=hd_num)
    inputs = {
        k: v.to(model.device)
    ...
    return outputs, im_mask


def process_image_intern(
    model,
    image_file: str,
    classes: List[str],
    prompt: str,
    ...
    with torch.no_grad():
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            first_outputs, im_mask = chat_likelihood(model, model.tokenizer, query=prompt, image=[image_file])

        probs = []
        for classname in tqdm(classes):
            inputs = model.tokenizer(classname, return_tensors="pt")
            input_ids = inputs.input_ids[0,:].numpy().tolist()  # batch size 1
        ...
        for idx in trange(len(data)):
             if all_classes.index(data[idx]["label"]) in range(10):
                 ...
                 if "blip" in model_id:
                     ...
                     choices = '\n'.join(['{} {}'.format(c_idx,c) for c_idx,c in  zip(['A.', 'B.', 'C.', 'D.', 'E.', 'F.', 'G.', 'H.', 'I.', 'J.'],classes)])
                     prompt = f"What is the year, make, and model of  the vehicle shown in the image? Consider the visual features of each.\n Choices:\n{choices}.\nThink step by step. Answer with the letter, extremely briefly, with a single charachter. Answer more briefly than you normally would."
                     probs = process_image_intern(
                         model,
                         data[idx]["image"],
                         #classes,
                         ['A','B','C','D','E','F','G','H','I','J'],
