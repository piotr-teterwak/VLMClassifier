        ...
        for idx in trange(len(data)):
             if all_classes.index(data[idx]["label"]) in range(10):
                 ...
                 if "blip" in model_id:
                     ...
                     choices2 = '\n'.join(['{} {}'.format(c_idx,c) for c_idx,c in  zip(['A.', 'B.', 'C.', 'D.', 'E.', 'F.', 'G.', 'H.', 'I.', 'J.'],reversed(classes))])
                     prompt2 = f"User:<image> What is the year, make, and model of  the vehicle shown in the image? Consider the visual features of each.\n Choices:\n{choices}.\nThink step by step. Answer with the letter. Be extremely brief, answer with a single charachter. Be briefer than you normally would. <end_of_utterance>\nAssistant:  "
                     probs2 = process_image_idefics(
                         model,
                         processor,
                         data[idx]["image"],
                         #classes,
                     ...
                     flattened_probs_1 = [prob[0] for prob in probs]
                     min_probs1 = min(flattened_probs_1)
                     flattened_probs_2 = [prob[0] for prob in reversed(probs2)]
                     min_probs2 = min(flattened_probs_2)

                     for p_idx in range(10):
                         probs[p_idx][0] = (flattened_probs_1[p_idx] - min_probs1) + (flattened_probs_2[p_idx] - min_probs2)/2.
