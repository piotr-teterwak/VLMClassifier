...
def get_confusion_matrix(filename, output_filename):
            ...
            for idx, prob in enumerate(item["probs"]):
                ...
                if False:
                    ...
                    probs.append(
                        (((
                            np.array(prob)
                            - np.array(calibrated_probs[idx]))/1)
                        ).sum()
                    )
   ...
   # cm_flat = cm.flatten()
   # top_confusions = np.argsort(-cm_flat)[:200000000]  # Get top 5 confusions
   ...
   #     if true_label_idx in [130,131]:
   #         print(f"True label: {true_label}, Predicted label: {pred_label}, Count: {cm[true_label_idx, pred_label_idx]}")

# Example usage
get_confusion_matrix('outputs/cars_llava7b.jsonl', 'output_confusion_matrix.png')
