...
def main(model_id, data_path, class_path, split, seed, output_path):
    ...
    with open(output_path, "a") as f:
        for idx in trange(len(data)):
             if all_classes.index(data[idx]["label"]) in range(10):
                 ...
                 if "blip" in model_id:
                     ...
                     choices = '\n'.join(['{} {}'.format(c_idx,c) for c_idx,c in  zip(['A.', 'B.', 'C.', 'D.', 'E.', 'F.', 'G.', 'H.', 'I.', 'J.'],classes)])
                     prompt = f"User:<image> What is the year, make, and model of  the vehicle shown in the image? Consider the visual features of each.\n Choices:\n{choices}.\nThink step by step. Answer with the letter. Be extremely brief, answer with a single charachter. Be briefer than you normally would. <end_of_utterance>\nAssistant:  "
                     ...
                     flattened_probs = [prob[0] for prob in probs]
                     min_prob = min(flattened_probs)
                     probs = [[prob - min_prob] for prob in flattened_probs]
                     top_4_indices = np.argsort(flattened_probs)[-4:][::-1]

                     # Get the corresponding classes and log-probs
                     top_4_classes = [classes[i] for i in top_4_indices]

                     # Generate new choices string for the top-4
                     choices_top_4 = '\n'.join(['{} {}'.format(c_idx, c) for c_idx, c in zip(['A.', 'B.', 'C.', 'D.'], top_4_classes)])

                     # Create the new prompt
                     new_prompt = f"User:<image> What is the year, make, and model of the vehicle shown in the image? Consider the visual features of each.\n Choices:\n{choices_top_4}.\nThink step by step. Answer with the letter. Be extremely brief, answer with a single character. Be briefer than you normally would. <end_of_utterance>\nAssistant:  "

                     # Recompute the probs for the top-4 classes
                     new_probs = process_image_idefics(
                         model,
                         processor,
                         data[idx]["image"],
                         ['A', 'B', 'C', 'D'],  # corresponding to the top-4 classes
                         new_prompt,
                     )

                     flattened_new_probs = [prob[0] for prob in new_probs]

                     # Initialize a new probability vector with -np.inf

                     # Map the new probabilities back to the original positions
                     for new_prob, p_idx in zip(new_probs, top_4_indices):
                         probs[p_idx][0] += new_prob[0] - min(flattened_new_probs)
                 else:
                     probs = process_image(
                         model,
                         processor,
